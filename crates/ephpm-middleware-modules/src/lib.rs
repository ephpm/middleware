...
//!
//! The sibling
//! `ephpm-middleware-{jwt,cors,ratelimit,redirect,security-headers}` crates
//! are thin cdylib shells: they re-export these types and add the `declare!`
//! C ABI glue, producing the loadable `.so`/`.dylib`/`.dll` artifacts for the
//! dynamic (dlopen) lane. The shells cannot be merged into one binary —
//! multiple copies of the same `ephpm_middleware_*` export symbols collide at
//! link time — which is exactly why the implementations live here.
